...
from pushmate.git import GitClient, GitTarget
...
def run_commit(max_chars: int):
    ...
    for filename, changes in invalid_files:
        confirmation = Prompt.ask(
            get_prompt(f"file {filename} has {changes} changes: include in commit?"),
            choices=["y", "N"],
            default="N",
        )
    ...
    generation_message = "generating commit message"
    message = None
    while not message:
        with console.status(get_status(generation_message)):
        ...
        if confirmation.lower() == "n":
            ...
            message = None
            generation_message = "regenerating commit message"

    with console.status(get_status("creating commit")):
        git_client.create_commit(message)
